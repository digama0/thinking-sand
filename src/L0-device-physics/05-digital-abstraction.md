...
One preview of a profound consequence. The progress claim (P) — transitions complete in bounded time — *provably cannot be made unconditional*: a classical theorem (Marino, 1981) shows any bistable device with continuous dynamics can be driven to sit arbitrarily long between its states. That failure mode is **metastability**, already met from the consumer's side in [L2/06](../L2-timing/06-boundaries.md); here it appears as it really is — not an engineering blemish but a topological necessity, the one permanently unclosable hole in the digital abstraction, and the reason the axiom ledger carries P1 forever.
