...
| S3 | The authored picorv32-IRQ specification is what was intended | L6 | **A** | **No standard exists to conform to** — picorv32's interrupt mechanism is custom (`getq`/`setq`/`retirq`/`maskirq`/`waitirq`, q-registers). This spec must be written, and its fidelity is unfalsifiable. The sharpest surviving specification axiom. |
...
| # | claim | layer | status |
|---|---|---|---|
| F1 | Hold is met at all corners | L2 | **FAILS.** 3 of 9 corners report `in2reg` hold failures (`signoff.rpt`). Hold is frequency-independent and unfixable post-silicon. Probably covered by the false-path declarations on `mprj_io[*]` — but nobody has checked. |
...
| F3 | The 159 SDC exceptions are justified | L2 | **UNVERIFIED.** 112 false paths + 45 case analyses + 2 clock groups. Each trades safety for frequency; no tool checks any. |
| F4 | Declared `set_case_analysis` modes cover all reachable configurations | L2 | **UNVERIFIED.** Some pin *flip-flop outputs* to constants. If a reachable state falls outside every declared mode, timing was never verified there. |
