...
| F7 | The pinned artifact carries the target core | L3–L5 | **FALSE as currently pinned.** The pinned `gl_caravel_core.v` / `rtl_caravel_core.v` contain the **VexRiscv/LiteX** management core (14,297 identifiers; zero picorv32). The book's target framing says picorv32; the netlist-level findings describe the VexRiscv build (housekeeping-level results are core-independent). Resolution pending: re-pin to a picorv32 Caravel build (`caravel_pico`) or restate the claim. |
