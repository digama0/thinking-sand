...
| E2 | Standard-cell Liberty tables match SPICE match silicon | **Discharged** — Route: L0/03 derives interval-valued tables (with inter-sample derivative bounds) from E1's interval device model by verified DAE enclosure. The PDK `.lib` then *exits the trusted base* and is demoted to an oracle for cross-checking. |
...
| # | claim | layer | status |
|---|---|---|---|
| F1 | Hold and setup are met at all corners | L2 | **UNESTABLISHED** — the flow's multi-corner signoff has not yet closed on the final layout. The hypothesis is live, not hypothetical: an independent hardening run of the same RTL measured hold *failing* at six of nine corners (fast and typical) with setup comfortable — the corner where this design's risk concentrates is known ([findings](findings.md)). |
| F2 | All loads/slews lie within the Liberty characterisation range | L2 | **AT RISK, measured** — the independent hardening run reported thousands of max-slew and hundreds of max-cap violations on stretched nets; outside the table the tool *extrapolates*, so affected timing numbers are vacuous rather than merely wrong. The primary flow's verdict is pending; the row closes only with a clean domain check. |
| F3 | The constraint set is complete and its exceptions justified | L2 | **FAILS as generated, sharply** — the emitted SDC declares **one clock**; the JTAG and serial-link clock domains are absent, so their paths and crossings are *unanalysed, not excepted* (L2/04). No false-path or case-analysis assertions exist to audit — the whole risk is the gap. Closure: complete the set, re-run signoff, discharge the crossings against the synchroniser inventory. |
| F5 | Register correspondence survives synthesis | L3 | **UNKNOWN, decidable at will** — the flow is ours to instrument; ρ is a logging patch away (L3/03). Yosys emits no SVF-equivalent by default, which is exactly the certification gap the project's endgame closes. |
| F7 | The artifact chain names one design | L3–L5 | **HOLDS BY CONSTRUCTION, checked by re-elaboration** — every layer's artifact descends from one pinned elaboration (Chisel → FIRRTL → SystemVerilog → netlist → layout), and the re-elaboration diff (L4/04) is the standing check that the chain has not drifted. The failure mode this row guards against — layers describing different builds — is structural for inherited artifacts and merely regressive here. |
| F8 | Independent signoff engines agree | L1/L2 | **FALSE, measured** — on the same layout, Magic DRC reported 0 violations while KLayout reported 4 (met4 width/spacing); LVS meanwhile caught a real power short the geometric engines disagreed about ([findings](findings.md)). Consequence: no single engine's verdict is treated as *the* verdict — G-checks are stated engine-independently, and agreement is a claim to measure per layout. |

(F4 — mode coverage — merged into F3: the generated constraints declare no modes, so completeness is the whole question. F6 — reachable clock configurations respect closure — retired: no software-reachable clock knob exists on this design; the clock-frequency condition is the *board's* obligation, filed under P6/X4.)
...
| M8 | **The two non-restoring modes: metastable saddle and oscillator phase** | L0/05, L2 | The two places restoration fails, and therefore the two places physical content survives to the ledger: the synchroniser's τ (unstable eigenvalue at the saddle) and — for designs with an on-die oscillator — the frequency and phase-diffusion coefficient (Floquet analysis of the limit cycle). Both are eigenvalue-enclosure problems over the interval device model — the same machinery as L0/03. This design consumes only the saddle half (its clock is board-supplied); discharging it converts P1's rate bound into a theorem modulo E1. Prior art is mature: interval Poincaré maps ([Zgliczyński](bibliography.md#zgliczynski-1997), [Galias](bibliography.md#galias-2001)) for the orbits; [Demir–Mehrotra–Roychowdhury](bibliography.md#demir-mehrotra-roychowdhury-2000) for phase noise. |
