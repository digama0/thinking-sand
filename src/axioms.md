...
| F7 | The pinned artifact carries the target core | L3–L5 | **RESOLVED (2026-08-02): the target now follows the silicon.** The pinned netlists carry the **VexRiscv/LiteX** management core; the book's target is restated accordingly — VexRiscv `MinDebugCache` (pipelined RV32I + I-cache + machine-mode CSRs + debug, identified from the GL's plugin registers), with picorv32 retained as the pinned *comparison* core the original scoping was stated against. Consequences priced in the layer retarget notes: L5 leaves its absence-table baseline (pipeline, cache now present), L4's census moves to machine-emitted sources, L6's interrupt story becomes the standard machine-mode subset. |
