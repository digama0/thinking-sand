...
**The encoding partition, checked — F8** (`tools/partition.py`): the spec-side patterns (49 instructions: RV32I + Zicsr + fences + `ecall`/`ebreak`/`mret`/`wfi` — WFI is implemented, a record addition) against the 20 legality cubes extracted from the shipped decoder's own `decode_LEGAL_INSTRUCTION` expression, compared exactly over all 2³² words by BDD. Result: **spec-only 0** — every spec-legal word is accepted — and **decoder-only 4,292,609**: the decoder accepts all 2²² words of reserved LOAD funct3=110, 98,304 reserved shift-immediate variants (nonzero high `funct7` bits on `slli`/`srli`/`srai`), and exactly one SYSTEM word — **`sret`**, which the minimised decoder catches in `mret`'s slot via a don't-care bit. These words *execute* (as whatever the don't-care-minimised control signals say) instead of trapping; filed as **F8**.
