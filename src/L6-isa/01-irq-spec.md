...
For this core the residue is mercifully small, and every piece of it is a measured fact of the [configuration record](../tools/config-record.py). The centrepiece is the **external-interrupt array**: 32 interrupt lines with a mask register and a pending register exposed as two custom CSRs (`0xBC0`/`0xFC0`), whose OR funnels into the standard external-interrupt pending bit — machinery from the [`ExternalInterruptArrayPlugin`](https://github.com/SpinalHDL/VexRiscv/blob/master/src/main/scala/vexriscv/plugin/ExternalInterruptArrayPlugin.scala), outside any ratified document. Around it, three smaller residues: the standard software and timer interrupts are **never pending** (their wires are tied to zero at [instantiation](https://github.com/efabless/caravel_mgmt_soc_litex/blob/503eda0790085712ffef7f4ad8934c7daed3237f/verilog/rtl/mgmt_core.v) — a fact the spec must state, since the imported model would otherwise permit them); the **reset vector arrives from a register** (LiteX's CSR-writable reset address) rather than a constant, so the spec's reset section is parameterised where the standard expects a platform constant; and the **debug unit** ([`DebugPlugin`](https://github.com/SpinalHDL/VexRiscv/blob/master/src/main/scala/vexriscv/plugin/DebugPlugin.scala)) can reach architectural state out of band — excluded from the spec by the debug-inactive conditionality (L5/04) rather than specified. (The reserved encodings the decoder accepts are *not* residue to author: [03](03-coverage.md)'s spec-UB clause assigns them unspecified behaviour, so no alias semantics needs writing.)

The trap machinery adds three **measured deviations** from the ratified semantics ([config-record](../tools/config-record.py), pinned by [`check-l6.py`](../tools/check-l6.py)) — places where the imported model says one thing and the silicon does another, so the `⊕`-extension must *override* rather than extend:

- **`ebreak` raises no breakpoint exception.** The shipped core has no cause-3 path at all: with a debug session active, `ebreak` halts into the debug unit (out of band, covered by the debug exclusion); with debug inactive — the normal case — it *retires as a no-op*. The spec must say so, because the imported model would trap.
- **`ecall` writes the instruction word into `mtval`** (the constant `0x00000073`), where the standard says zero for traps that carry no information.
- **No access fault is reachable — bus errors are invisible entirely.** The core's own Wishbone bridges tie both error inputs to zero (`iBus_rsp_payload_error = 0`, `dBus_rsp_error = 0`), discarding the ERR wiring the SoC dutifully drives into the core's ports; the MMU exception path is likewise tied off. The cause-1/5/7 logic inside the plugins exists but is structurally dead, so a failing access silently completes with whatever the bus returned. (Consistent with L7/03: the interconnect *acks* unmapped accesses after its timeout rather than erroring — on this SoC the two mechanisms agree that software never sees a bus problem as a trap.) The complete reachable synchronous trap surface is therefore causes **0, 2, 4, 6, 11** — plus the external interrupt, the only live asynchronous cause.
