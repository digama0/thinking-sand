...
4. **Delivery gating**: a trap step is taken exactly when `mstatus.MIE ∧ (mip ∧ mie) ≠ 0` at a boundary (or a synchronous exception retires); no lost interrupts — a pending line stays pending until handled or unmasked, with "handled *eventually*" conditional on software, hence a `Sys(F)`-level claim, split explicitly.
