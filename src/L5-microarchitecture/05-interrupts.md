...
The synchronous side is narrower than the standard too: the measured trap surface is causes {0, 2, 4, 6, 11} only — no access fault is reachable (the core's bus bridges tie their error inputs to zero), `ebreak` raises nothing (debug-halt or NOP), and `mtvec` is direct-mode-only with no reset value. The measured record and its deviations are L6/01's and L6/02's; this chapter's lemmas consume them.
...
The imported half is anchored by the standard's own artifacts: the [riscv-arch-test](https://github.com/riscv-non-isa/riscv-arch-test) suite run against the imported model, and [riscv-formal](https://github.com/YosysHQ/riscv-formal)'s VexRiscv checks for what "an interrupt retired correctly" observably means. The authored residue is anchored the usual way: formalise the plugin's documentation and the [LiteX](https://github.com/enjoy-digital/litex) interrupt-handling conventions *first*, then diff against RTL behaviour; the shipped firmware's interrupt usage is the working-software corpus that the spec must make correct — and it is extracted and pinned ([`fwanchors.py`](../tools/fwanchors.py)): `crt0` writes `mtvec` before setting `mie` (the discipline the missing reset value forces), the corpus writes the mask CSR and only ever *reads* the pending CSR (the trapping write is never exercised), and one stale expectation surfaces — `mie = 0x880` sets MTIE, which is structurally dead here since the timer arrives through the array.
