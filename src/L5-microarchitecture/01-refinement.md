...
The last two rows are why the bus contract must carry a **latency bound**: with an unbounded Wishbone `ACK` the measure does not exist and the refinement silently weakens from "instructions retire" to "instructions retire if the bus answers." State it conditionally: `m` exists given the contract's bound `B`; the SoC proof (L7/B2) discharges `B` for the actual fabric, with the flash-XIP fetch path as the worst case.
...
3. The stutter-source table's bounds, extracted from the [pinned source](https://github.com/efabless/caravel_mgmt_soc_litex/blob/503eda0790085712ffef7f4ad8934c7daed3237f/verilog/rtl/VexRiscv_MinDebugCache.v) (with [00](00-microarchitecture.md)'s stage-graph extraction), and the conditional-on-`B` formulation with `B` named as an L7 deliverable.
