...
There is also a fact about *where this information lives* that a newcomer would not guess: nowhere checkable. The memory map's authoritative source is a **Python program**. Caravel's management SoC is built with LiteX, a framework in which hardware is described by Python code that *generates* Verilog — so the map exists as generator source, as generated RTL, as generated firmware headers, as hand-written `#define`s, and as human-written documentation: five artifacts with no tool upstream checking their agreement. This is normal practice, not a Caravel quirk — which is why this chapter's first deliverable is a diff of all five ([`memmap.py`](../tools/memmap.py)), and why that diff finds real disagreements in shipped artifacts ([findings](../findings.md)).
...
Two standing observations. First, the memory map's authoritative source is a **Python generator** — spec-by-generator, so read `caravel.py`, not the RST. (The generator can also be *re-run*: [`regenerate-mgmt-core.sh`](../tools/regenerate-mgmt-core.sh) rebuilds the RTL and the firmware headers from it, and every fact this chapter extracts survives the round trip — [findings](../findings.md#replicating-the-generator).) Second, [`memmap.py`](../tools/memmap.py) extracts the shipped decode (seven windows from `mgmt_core.v`'s `slave_sel` comparisons, plus the 20 LiteX CSR banks inside the `csr` window) and diffs it against the other three artifacts — and drift is exactly what it finds ([findings](../findings.md)): the `hk` window is declared 3 MB but decoded as 4 MB (LiteX rounds decode windows to powers of two), four live `defs.h` pointers target unmapped space, `csr-defs.h` names a machine CSR (`CSR_DCACHE_INFO`, 0xCC0) the shipped core does not decode, and two `defs.h` macros name CSR symbols the generated header never emits. The generated [`csr.h`](https://github.com/efabless/caravel_mgmt_soc_litex/blob/503eda0790085712ffef7f4ad8934c7daed3237f/verilog/dv/generated/csr.h) is the one artifact that names each CSR bank semantically, and it matches the RTL decode bank for bank — which is what lets the 49 symbolic firmware pointers be checked at all. The unmapped-space semantics matter to [02](02-bus-contract.md)'s contract: an access to a hole does not trap — the shared interconnect **times out after 10⁶ cycles**, acks with `0xFFFF_FFFF`, and counts a `bus_errors` CSR — so a stray firmware pointer costs a ~100 ms stall at 10 MHz, and the address-decode model must say so.
