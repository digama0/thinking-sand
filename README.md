...
Effort figures are for one competent person and are the sums of the per-subcomponent estimates (the pre-decomposition guesses ran ~2× higher; the decomposition revised them). Shared infrastructure — the proof framework, automation, checker integrations — is priced separately in MAIN at 1–2 yr; sequential total ≈ 8–15 person-years. All eight layers are decomposed into numbered subcomponent files, one per major proof obligation. Each layer README opens with a **spec block** — spec below, spec above, theorem or definition — keyed to [MAIN's spec tower](src/MAIN.md#the-spec-tower).
