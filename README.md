...
The core inside the pinned artifacts is **VexRiscv in its `MinDebugCache` configuration** — a pipelined RV32I core with an instruction cache, machine-mode CSRs, and a debug unit, generated by [SpinalHDL](https://github.com/SpinalHDL/SpinalHDL) and integrated by a [LiteX](https://github.com/enjoy-digital/litex)-generated SoC; the identification from the netlist's own plugin registers is finding F7, pinned by the checkers. [caravel_mgmt_soc_litex](https://github.com/efabless/caravel_mgmt_soc_litex) also ships [picorv32](https://github.com/YosysHQ/picorv32) as a configuration option — hand-written and far friendlier to invariant authoring — but the fabricated netlist is the VexRiscv build, and **the target follows the silicon**: simplicity is not the goal, realism is. The costs are priced where they bite (machine-emitted Verilog with mangled names; a pipeline and a cache in L5; no written-down IR semantics for the generators); picorv32 stays pinned as the comparison core.
